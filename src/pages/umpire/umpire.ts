...
export class UmpirePage
{
  ...
  static height1 =0;
  static octant = 0;
  static innings = "FirstInnings";
  static firstInn_score = 0;
  inn = "FirstInnings";
  ...
  constructor(public navCtrl: NavController, public toastCtrl: ToastController, public fdb: AngularFireDatabase, platform : Platform)
  {
    ...
    console.log("Innings: " + this.inn);
    this.name.take(1).subscribe(data =>
    {
        console.log("Match Ptr: " + data);
        this.key.MatchKey= data.matchPtr;
    this.name = this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/`+ this.inn + `/Score/`);
  ...
  pushdata()
  {
    ...
    console.log("Here: " + UmpirePage.innings);
    this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/`+ UmpirePage.innings + `/Score`).take(1).subscribe(data =>
      {
        console.log("Get ball ptr: " + data.ballPtr);
        val = data.numOfOvers*6;
        console.log("Num: " + val);
  ...
  endMatch(){
    if(UmpirePage.innings == "FirstInnings"){
      this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/` + UmpirePage.innings + `/Score/`).take(1).subscribe(data =>
        {
          console.log("toast ball Ptr: " + data.ballPtr);
          UmpirePage.firstInn_score = data.totalRuns;
          this.inn = "SecondInnings";
          console.log("First Score: " + UmpirePage.firstInn_score);
        });// Finds out the corrent matchPtr
      var bool = confirm("End Innings?");
      if(bool){
        UmpirePage.innings = "SecondInnings";
        this.score.totalRuns = 0;
        this.score.ballPtr = 1;
        this.score.totalWickets = 0;
        this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/` + UmpirePage.innings + `/Score/totalRuns/` )
        .set(0);
       this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/` + UmpirePage.innings + `/Score/totalWickets/` )
        .set(0);
       this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/` + UmpirePage.innings + `/Score/totalOvers/` )
        .set("0.0");
       this.fdb.object(`/Matches/` + this.key.MatchKey + `/MatchStats/` + UmpirePage.innings + `/Score/ballPtr/` )
        .set(1);
      }
      return;
    }
  ...
  updateTotalScore()
  {
   ...
   console.log("TOTal Num: " + this.score.totalRuns);
   console.log(this.key.MatchKey);
   if(UmpirePage.innings != "FirstInnings" && UmpirePage.firstInn_score != 0 && this.score.totalRuns > UmpirePage.firstInn_score){
    this.endMatch();
   }
   var val = 0;
   var wickets = 0;
  ...
  clear(){
    ...
    document.getElementById('wide').style.background= '#002855';
    document.getElementById('wide').style.color = 'dark';
    document.getElementById('extra').style.background= '#002855';
    document.getElementById('extra').style.color = 'danger';
  ...
  wide_button() {
    document.getElementById('wide').style.background= 'dark';
    document.getElementById('wide').style.color = '#002855';
    document.getElementById('extra').style.background= '#002855';
    document.getElementById('extra').style.color = 'danger';
    document.getElementById('run_button0').style.color = 'goldenrod';
    document.getElementById('run_button0').style.background = '#002855';
  ...
  extra_button() {
    document.getElementById('wide').style.background= '#002855';
    document.getElementById('wide').style.color = 'dark';
    document.getElementById('extra').style.background= 'danger';
    document.getElementById('extra').style.color = '#002855';
    document.getElementById('run_button0').style.color = 'goldenrod';
    document.getElementById('run_button0').style.background = '#002855';
    document.getElementById('run_button1').style.color = 'goldenrod';
    document.getElementById('run_button1').style.background = '#002855';
